<template>
  <v-expansion-panels focusable v-model="panel" multiple>
    <v-expansion-panel v-for="item in itemsData" :key="item.rowId" class="my-1">
...
export default {
  ...
  watch: {
    ...
    // itemsData(newV) {
    //  console.log(newV.findIndex((item) => item.rowId === this.newItemId));
    //  this.panel = [newV.findIndex((item) => item.rowId === this.newItemId)];
    // },
  },
  computed: {
